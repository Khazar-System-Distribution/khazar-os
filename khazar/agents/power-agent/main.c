...
#define MODULE "power-agent"
#define AGENT_NAME "power-agent"
#define AGENT_VERSION "0.1.0"
#define ORCH_SOCKET "/tmp/ai-orch.sock"
#define AGENT_SOCKET "/run/ai-power-agent.sock"
...
static int register_with_orchestrator(void) {
    ...
    snprintf(msg, sizeof(msg),
        ...
        "\"socket\":\"%s\",\"capabilities\":[\"system_management\"]}",
...
static void handle_client(int client_fd, const char *data, size_t len) {
    ...
    if (strstr(query, "shutdown") || strstr(query, "kapat") || strstr(query, "sondur") || strstr(query, "söndür")) {
        if (!strstr(query, "wifi")) {
            log_warn(MODULE, "shutdown requested");
            if (run_command("systemctl poweroff") == 0) {
                success = 1;
                snprintf(message, sizeof(message), "shutdown initiated");
            } else {
                snprintf(message, sizeof(message), "shutdown failed");
            }
        }
    } else if (strstr(query, "reboot") || strstr(query, "yeniden") || strstr(query, "başlat")) {
        if (run_command("systemctl reboot") == 0) {
            success = 1;
            snprintf(message, sizeof(message), "reboot initiated");
        } else {
            snprintf(message, sizeof(message), "reboot failed");
        }
    } else if (strstr(query, "suspend") || strstr(query, "uyku") || strstr(query, "askıda")) {
        if (run_command("systemctl suspend") == 0) {
            success = 1;
            snprintf(message, sizeof(message), "suspend initiated");
        } else {
            snprintf(message, sizeof(message), "suspend failed");
        }
    } else if (strstr(query, "hibernate") || strstr(query, "hazırda") || strstr(query, "hazırda beklet")) {
        if (run_command("systemctl hibernate") == 0) {
            success = 1;
            snprintf(message, sizeof(message), "hibernate initiated");
        } else {
            snprintf(message, sizeof(message), "hibernate failed");
        }
    } else if (strstr(query, "lock") || strstr(query, "kilitle") || strstr(query, "ekranı kilitle")) {
        if (run_command("loginctl lock-session") == 0) {
            success = 1;
            snprintf(message, sizeof(message), "session locked");
        } else {
            snprintf(message, sizeof(message), "lock failed");
        }
    } else if (strstr(query, "logout") || strstr(query, "cik") || strstr(query, "çık") || strstr(query, "oturumu kapat")) {
        if (run_command("loginctl terminate-session") == 0) {
            success = 1;
            snprintf(message, sizeof(message), "logout initiated");
        } else {
            snprintf(message, sizeof(message), "logout failed");
        }
    }
...
int main(void) {
    ...
    log_info(MODULE, "AI Power Agent v%s starting...", AGENT_VERSION);
    ...
    log_info(MODULE, "capabilities: system_management");
