...
static int register_with_orchestrator(void) {
    ...
    snprintf(msg, sizeof(msg),
        ...
        "\"socket\":\"%s\",\"capabilities\":[\"network_management\"]}",
...
static int execute_action(const char *action, const char *target) {
    ...
    if (strcmp(action, "wifi_enable") == 0) {
        snprintf(cmd, sizeof(cmd), "nmcli radio wifi on");
    } else if (strcmp(action, "wifi_disable") == 0) {
        snprintf(cmd, sizeof(cmd), "nmcli radio wifi off");
    } else if (strcmp(action, "status") == 0) {
        snprintf(cmd, sizeof(cmd), "nmcli device status");
    } else if (strcmp(action, "connect") == 0) {
        if (!target || !target[0]) return -1;
        snprintf(cmd, sizeof(cmd), "nmcli device wifi connect \"%s\"", target);
    } else {
        return -1;
    }
...
static void handle_client(int client_fd, const char *data, size_t len) {
    ...
    if (strstr(query, "wifi") && (strstr(query, "open") || strstr(query, "ac") || strstr(query, "aç") ||
        strstr(query, "enable") || strstr(query, "etkinle"))) {
        if (execute_action("wifi_enable", NULL) == 0) {
            success = 1;
            snprintf(message, sizeof(message), "wifi enabled");
        } else {
            snprintf(message, sizeof(message), "failed to enable wifi");
        }
    } else if (strstr(query, "wifi") && (strstr(query, "close") || strstr(query, "kapat") ||
        strstr(query, "disable") || strstr(query, "devre di"))) {
        if (execute_action("wifi_disable", NULL) == 0) {
            success = 1;
            snprintf(message, sizeof(message), "wifi disabled");
        } else {
            snprintf(message, sizeof(message), "failed to disable wifi");
        }
    } else if (strstr(query, "status") || strstr(query, "durum")) {
        execute_action("status", NULL);
        success = 1;
        snprintf(message, sizeof(message), "network status queried");
    } else if (strstr(query, "connect") || strstr(query, "baglan") || strstr(query, "bağlan")) {
        char ssid[128] = {0};

        const char *keywords[] = {"connect", "baglan", "bağlan", NULL};
        for (int i = 0; keywords[i]; i++) {
            const char *pos = strstr(query, keywords[i]);
            if (pos) {
                const char *after = pos + strlen(keywords[i]);
                while (*after == ' ') after++;
                strncpy(ssid, after, sizeof(ssid) - 1);
                break;
            }
        }

        char *t = ssid + strlen(ssid) - 1;
        while (t >= ssid && *t == ' ') { *t = '\0'; t--; }

        if (ssid[0]) {
            if (execute_action("connect", ssid) == 0) {
                success = 1;
                snprintf(message, sizeof(message), "connected to %s", ssid);
            } else {
                snprintf(message, sizeof(message), "failed to connect to %s", ssid);
            }
        }
    }
...
int main(void) {
    ...
    log_info(MODULE, "AI Network Agent v%s starting...", AGENT_VERSION);
    ...
    log_info(MODULE, "capabilities: network_management");
